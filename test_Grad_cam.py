    ...
    for (path, img, im0s, vid_cap), (path_, img2, im0s_, vid_cap_) in zip(dataset, dataset2):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                flag = 0
                # Write results
                sorted_det = sorted(det, key=lambda x: x[4], reverse=True)
...
def detect_with_gradcam(opt):
    source1, source2, weights, view_img, save_txt, imgsz = opt.source1, opt.source2, opt.weights, opt.view_img, opt.save_txt, opt.img_size

    save_img = not opt.nosave and not source1.endswith('.txt')
    webcam = source1.isnumeric() or source1.endswith('.txt') or source1.lower().startswith(
        ('rtsp://', 'rtmp://', 'http://', 'https://'))
    ...
    def hook_fn(module, input, output):
        print("hook_fn activate!")
        global feature_map
        feature_map = output  # 保存输出特征图
    # Grad-CAM preparation
    target_layer = model.model[17] # Adjust based on YOLO architecture
    ...
    target_layer.register_backward_hook(save_grad_output)

    dataset = LoadImages(source1, img_size=imgsz, stride=stride)
    dataset2 = LoadImages(source2, img_size=imgsz, stride=stride)

    for (path, img, im0s, vid_cap), (path_, img2, im0s_, vid_cap_) in zip(dataset, dataset2):
        img = torch.from_numpy(img).to(device).float() / 255.0  # 转换为浮点数并归一化
        img2 = torch.from_numpy(img2).to(device).float() / 255.0  # 转换为浮点数并归一化
        ...
        if not pred.requires_grad:
            raise RuntimeError("Model output does not support gradients. Check model definition.")
        # Backpropagation for Grad-CAM
        one_hot = torch.zeros_like(pred)
        one_hot[0, 0] = 1  # Focus on the first detected object
        print(pred.requires_grad)  # 应该返回 True
        print(pred.grad_fn)  # 应该不为 None
        #pred.backward()
        
        # Extract Grad-CAM
        feature_map = pred.output[0].detach().cpu().numpy()
        gradient = grad_outputs[0][0].detach().cpu().numpy()
        cam = np.mean(gradient, axis=(1, 2))[:, None, None] * feature_map
        cam = np.maximum(cam.sum(axis=0), 0)
        cam = cv2.resize(cam, (im0s.shape[1], im0s.shape[0]))
        cam = (cam - cam.min()) / (cam.max() - cam.min())

        # Overlay Grad-CAM on image
        heatmap = cv2.applyColorMap(np.uint8(255 * cam), cv2.COLORMAP_JET)
        overlay = heatmap * 0.4 + im0s * 0.6
        save_path = str(save_dir / f"heatmap_{Path(path).stem}.jpg")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='./ckpts/yolov5l_llvip.pt', help='model.pt path(s)')
    parser.add_argument('--source1', type=str, default='./data/spetial', help='source')  # file/folder, 0 for webcam
    parser.add_argument('--source2', type=str, default='./data/spetial_infrared', help='source')  # file/folder, 0 for webcam
