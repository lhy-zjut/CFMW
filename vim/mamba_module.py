...
class WithBias_LayerNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        # self.weight = Parameter(self.weight.to('cuda'))
        # self.bias = Parameter(self.bias.to('cuda'))
...
class CrossMamba(nn.Module):
    def __init__(self, dim):
        ...
        self.cross_mamba = Mamba(dim)
        self.norm = LayerNorm(dim,'with_bias')
        self.dwconv = nn.Conv2d(dim, dim, kernel_size=3, padding=1, groups=dim)
    def forward(self,ms):
        ms = self.norm(ms)
        global_f = self.cross_mamba(self.norm(ms),extra_emb=self.norm(ms))
        B,HW,C = global_f.shape
        ms = global_f.transpose(1, 2).view(B, C, 8, 16)
...
class TokenSwapMamba(nn.Module):
    def __init__(self, dim):
        ...
        self.msencoder = Mamba(dim)
        self.panencoder = Mamba(dim)
