...
class Mamba(nn.Module):
    ...
    def forward(self, hidden_states, inference_params=None,extra_emb=None):
        ...
        if extra_emb is None:
            ...
            # self.in_proj.weight = Parameter(self.in_proj.weight.to('cuda'))
            # self.in_proj_extra.weight = Parameter(self.in_proj_extra.weight.to('cuda'))
            xz = rearrange(
                self.in_proj.weight @ rearrange(hidden_states, "b l d -> d (b l)"),
                "d (b l) -> b d l",
                l=seqlen,
        ...
        if self.use_fast_path and inference_params is None:  # Doesn't support outputting the states
            if self.bimamba_type == "v1":
                ...
                if not self.if_devide_out:
                    # self.out_proj.weight = Parameter(self.out_proj.weight.to('cuda'))
