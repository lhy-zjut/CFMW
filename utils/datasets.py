...
def img2label_path_visible(img_paths):
    # Define label paths as a function of image paths
    sa, sb = os.sep + 'visible' + os.sep, os.sep + 'labels' + os.sep  # /images/, /labels/ substrings
    return ['txt'.join(x.replace(sa, sb, 1).rsplit(x.split('.')[-1], 1)) for x in img_paths]

def img2label_path_infrared(img_paths):
    # Define label paths as a function of image paths
    sa, sb = os.sep + 'infrared' + os.sep, os.sep + 'labels' + os.sep  # /images/, /labels/ substrings
    return ['txt'.join(x.replace(sa, sb, 1).rsplit(x.split('.')[-1], 1)) for x in img_paths]
...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(self, path_rgb, path_ir, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        # Check cache
        # Check rgb cache
        self.label_files_rgb = img2label_path_visible(self.img_files_rgb)  # labels
        # print(self.label_files)
        ...
        # Check ir cache
        self.label_files_ir = img2label_path_infrared(self.img_files_ir)  # labels
        # print(self.label_files)
