...
class EMAHelper(object):
    ...
    def update(self, module):
        ...
        for name, param in module.named_parameters():
            if param.requires_grad:
                # 确保所有数据都在同一个设备上（这里假设在 `cuda:0` 上），将 param 和 self.shadow[name] 移动到相同设备
                device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
                param.data = param.data.to(device)
                self.shadow[name].data = self.shadow[name].data.to(device)
...
def noise_estimation_loss(model, x0, t, e, b):
    ...
    x = x0[:, 3:, :, :] * a.sqrt() + e * (1.0 - a).sqrt() # 对应原文中的x_t
    output = model(torch.cat([x0[:, :3, :, :], x], dim=1), t.float()) # 这里两个输入，前一个输入是受天气影响的图像，后一个是加噪声之后的
    return (e - output).square().sum(dim=(1, 2, 3)).mean(dim=0) # 高斯噪声-预测的结果=原始图像
            ...
            for i, (x, y) in enumerate(train_loader):
                ...
                if self.step % self.config.training.snapshot_freq == 0 or self.step == 1:
                    utils.logging.save_checkpoint({
                        ...
                        'params': self.args,
                        'config': self.config
                    }, filename=os.path.join('/home/lhy/WeatherDiffusion/ckpts','allweather_',str({epoch})))
