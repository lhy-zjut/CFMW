...
class AllWeather:
    ...
    def get_loaders(self, parse_patches=True, validation='allweather'):
        if validation == 'raindrop':
            ...
            filename = 'test1.txt'
        elif validation == 'snow':   # snow
            print("=> evaluating snowtest100K-L...")
            path = os.path.join(self.config.data.data_dir, 'data', 'snow100k')
            filename = 'snowtest100k_L.txt'

        else:  # allweather
            print("=> evaluating allweather...")
            path = os.path.join(self.config.data.data_dir, 'Allweather')
            filename = 'allweather.txt'

        train_dataset = AllWeatherDataset(os.path.join(self.config.data.data_dir,'Allweather'),
