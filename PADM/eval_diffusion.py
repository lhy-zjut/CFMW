...
def main():
    ...
    print(model.diffusion.model)
    model_graph = make_dot(model, params=dict(model.named_parameters()))

    # 保存为图片
    model_graph.render("diffusion_unet_architecture", format="png")
    # start = time.time()
    # num_images = model.restore(val_loader, validation=args.test_set, r=args.grid_r)
    # end = time.time()
    # average_time = (end - start)/ num_images
    # print(f"average time for each image: {average_time}")
